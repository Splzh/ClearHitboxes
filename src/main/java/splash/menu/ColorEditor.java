...
public class ColorEditor extends Screen {
    ...
    public void render(DrawContext context, int mouseX, int mouseY, float tickDelta) {
        ...
        /** --- Settings Overlay --- */
        context.fill(width/2-20,menuY2+25,menuX-5,menuY -5, shade.getRGB());
        context.fill(menuX-8,height/2-56,menuX-9,height/2-1, darkGray.getRGB());
        context.fill(menuX-8,height/2+19,menuX-9,height/2+91, darkGray.getRGB());
        context.fill(width/2-20+2,height/2-73,menuX-5-2,height/2-60, darkGray.getRGB());
        context.fill(width/2-20+2,height/2+2,menuX-5-2,height/2+15, darkGray.getRGB());
        context.fill(width/2-16,height/2-56,width/2-17,height/2-1, darkGray.getRGB());
        context.fill(width/2-16,height/2+19,width/2-17,height/2+91, darkGray.getRGB());

        /** --- Button Label --- */
        context.fill(width/2-20+2+22, height/2-100+6, width/2-20+2+22+45, height/2-100+6+15, shade.getRGB());

        /** --- Finishing Touches --- */
        MenuUtils.drawPreview(context, menuX2+50, height/2+70);
        Render.drawShadowString(context, 1.2f, "Clear Hitboxes",  menuX2+10, menuY2+10,Color.white.getRGB());
        context.getMatrices().push();
        context.getMatrices().translate((float)(menuX2+10+textRenderer.getWidth("Clear Hitboxes")*100/80-10), menuY2+3F, 0.0F);
        context.getMatrices().multiply(RotationAxis.POSITIVE_Z.rotationDegrees(20.0F));
        context.getMatrices().pop();

        /** --- Settings Information --- */
        Render.drawString(context, 1f, "Basic Colors",  width/2-10+60-textRenderer.getWidth("Basic Colors")/2, height/2-70, extraColor.getRGB());
        Render.drawShadowString(context,1.0f, "Red", (float) (width/2-10), height/2-55, Color.white.getRGB());
        Render.drawShadowString(context,1.0f, "Green", (float) (width/2-10), height/2-40, Color.white.getRGB());
        Render.drawShadowString(context, 1.0f, "Blue", (float) (width/2-10), height/2-25, Color.white.getRGB());
        Render.drawShadowString(context, 1.0f, "Alpha", (float) (width/2-10), height/2-10, Color.white.getRGB());
        if(ClearHitboxesConfig.enableChroma){
            Render.drawShadowString(context, 1f, "Chroma Settings", width/2-10+60-textRenderer.getWidth("Chroma Settings")/2, height/2+05, ColorUtils.rainbow(0*300));
        } else{
            Render.drawString(context, 1f, "Chroma Settings", width/2-10+60-textRenderer.getWidth("Chroma Settings")/2, height/2+05, light.getRGB());
        }
        Render.drawShadowString(context,0.8f, "Enable Chroma", (float) (width/2-10), height/2+20, Color.white.getRGB());
        Render.drawShadowString(context, 0.8f, "Enable Gradient", (float) (width/2-10), height/2+35, Color.white.getRGB());
        Render.drawShadowString(context, 0.8f, "Gradient Speed", (float) (width/2-10), height/2+50, Color.white.getRGB());
        Render.drawShadowString(context, 0.7f, "Chroma Brightness", (float) (width/2-10), height/2+65, Color.white.getRGB());
        Render.drawShadowString(context, 0.8f, "Chroma Delay", (float) (width/2-10), height/2+80, Color.white.getRGB());
