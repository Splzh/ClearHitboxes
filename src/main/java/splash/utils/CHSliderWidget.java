...
public abstract class CHSliderWidget extends SliderWidget {
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawCenteredTextWithShadow(matrices,textRenderer, textRenderer.trimToWidth(getMessage(), width).getString(), this.getX() + width / 2, this.getY() + (height - 8) / 2, Color.WHITE.getRGB() | MathHelper.ceil(alpha * 255.0F) << 24);
        if (!active) {
    ...
    protected void renderBg(MatrixStack matrices, MinecraftClient client, int mouseX, int mouseY) {
        ...
        drawCenteredTextWithShadow(matrices,textRenderer, textRenderer.trimToWidth(getMessage(), width).getString(), this.getX() + width / 2, this.getY() + (height - 8) / 2, 0x00FFFFFF | MathHelper.ceil(alpha * 255.0F) << 24);
    }
