...
public abstract class CHButtonWidget extends PressableTextWidget {
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        if(isHovered()) {
            ...
            fill(matrices,x1, y1, x2, y2, BACKGROUND.getRGB());
            fill(matrices,x1, y1, x2, y1 + 1, color.getRGB());
            fill(matrices,x1, y1 + 1, x1 + 1, y2 - 1, color.getRGB());
            fill(matrices,x2, y1 + 1, x2 - 1, y2 - 1, color.getRGB());
        }

        fill(matrices,x1,     y2,     x2,     y2 - 1, color.getRGB());

        overflowTextRenderer.render(matrices, textRenderer, getMessage(), delta, color.getRGB());
        if (!active) {
