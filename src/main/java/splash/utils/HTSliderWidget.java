...
public abstract class HTSliderWidget extends SliderWidget {
    ...
    public void renderButton(DrawContext context, int mouseX, int mouseY, float delta) {
        ...
        context.fill(x1,     y1,     x2,     y1 + 1, shadow.getRGB());
        context.fill(x1,     y2,     x2,     y2 - 1, light.getRGB());
        context.fill(x1,     y1 + 1, x1 + 1, y2 - 1, shadow.getRGB());
        context.fill(x2,     y1 + 1, x2 - 1, y2 - 1, light.getRGB());
        int j = active ? 0x00FFFFFF : 0x00A0A0A0;
        Text message = getMessage();
        context.drawCenteredTextWithShadow(textRenderer, (Text) textRenderer.getTextHandler().trimToWidth(message, width, message.getStyle()), this.getX() + width / 2, this.getY() + (height - 8) / 2, j | MathHelper.ceil(alpha * 255.0F) << 24);
        if (!active) {
        ...
        context.fill(getX()+1, YY + 1, XX, Y3 - 1, color.getRGB());
        context.drawTexture(circle, XX-10, YY-10, 0, 0,27,27,27, 27);

        if(isHovered()){
            RenderSystem.setShaderTexture(0, bubble);
            MinecraftClient.getInstance().getTextureManager().bindTexture(bubble);
            context.drawTexture(bubble, XX-7, YY-17, 0, 0,18,18,18, 18);
            Render.drawString(context,0.8f, ""+Math.round(value*10)/10D, (XX-textRenderer.getWidth(""+Math.round(value*10)/10D)+11),  YY-12, Color.black.getRGB());
        }
    }
