...
public abstract class HTSliderWidget extends SliderWidget {
    ...
    public void renderButton(MatrixStack matrices, int mouseX, int mouseY, float delta) {
        ...
        drawTextWithShadow(matrices,textRenderer, (Text) textRenderer.getTextHandler().trimToWidth(message, width, message.getStyle()), this.getX() + width / 2, this.getY() + (height - 8) / 2, j | MathHelper.ceil(alpha * 255.0F) << 24);
        if (!active) {
